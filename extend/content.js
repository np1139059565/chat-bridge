...
(function () {
  ...
  let dialogHidden = true;   // 对话框是否隐藏；每次页面加载默认隐藏，仅用户点图标时展开
  let siteKey = '';          // 当前网站标识（按站点隔离数据与设置）
  let profileId = '';        // 当前生效的站点规则（glm / deepseek / custom）
  let panelSide = 'right';   // 抽屉挂靠侧：right / left（持久化）
  ...
  // 与自动化测试脚本一致：设值走原生 setter（兼容 Vue/React 的响应式），
  // 再派发 input 事件，等状态同步后派发 Enter 键事件触发发送。
  function findInputBox() {
    const sel = activeProfile().inputSelector;
    if (!sel) return null;
    return document.querySelector(sel);
  }

  function pasteToWebpageAI(text) {
    const ta = findInputBox();
    if (!ta) {
      warn('pasteToWebpageAI: 未找到网页 AI 输入框');
      post({ type: 'auto_send_result', ok: false, msg: '未找到网页 AI 输入框' });
      return;
    }
    ...
    setTimeout(() => {
      ...
      post({ type: 'auto_send_result', ok: true, msg: '已回传结果到网页 AI' });
    }, 500);
  }
  ...
  const PANEL_RADIUS_RIGHT = '12px 0 0 12px';          // 右侧挂靠：只圆左侧悬浮边
  const PANEL_RADIUS_LEFT = '0 12px 12px 0';           // 左侧挂靠：只圆右侧悬浮边
  const PANEL_SHADOW_RIGHT = '-4px 0 16px rgba(0,0,0,.25)';  // 右侧挂靠：向左投影
  const PANEL_SHADOW_LEFT = '4px 0 16px rgba(0,0,0,.25)';    // 左侧挂靠：向右投影
  const PANEL_Z = 2147483647;                          // 取层级上限，确保不被宿主网页元素遮挡

  // 面板定位样式：右侧挂靠 right:0，左侧挂靠 left:0；圆角与投影方向随挂靠侧翻转
  function panelPositionCss() {
    if (panelSide === 'left') {
      return 'position:fixed;top:0;left:0;width:' + panelWidth() +
        ';height:100vh;border:0;border-radius:' + PANEL_RADIUS_LEFT +
        ';box-shadow:' + PANEL_SHADOW_LEFT +
        ';background:' + PANEL_BG +
        ';z-index:' + PANEL_Z + ';';
    }
    return 'position:fixed;top:0;right:0;width:' + panelWidth() +
      ';height:100vh;border:0;border-radius:' + PANEL_RADIUS_RIGHT +
      ';box-shadow:' + PANEL_SHADOW_RIGHT +
      ';background:' + PANEL_BG +
      ';z-index:' + PANEL_Z + ';';
  }
  ...
  function applyPanelSide() {
    ...
    if (f) f.style.cssText = panelPositionCss();
  }
  ...
  function inject() {
    ...
    iframe.style.cssText = panelPositionCss();
  ...
  function setDialogVisible(visible) {
    ...
    log('setDialogVisible: visible=' + visible);
  }
  ...
  function extractBlocks(root) {
    ...
    messages.forEach(stampCodeBlockIds);
    return messages;
  }
  ...
  function stampCodeBlockIds(msg) {
    const blocks = msg.blocks || [];
    const digest = blocks.map(blockDigest);
    const fp = hashStr(msg.role + '|' + (msg.name || '') + '|' + JSON.stringify(digest));
    blocks.forEach((b, i) => {
      if (b.type === 'code') {
        b.id = 'c' + hashStr(fp + '|' + i + '|' + (b.lang || '') + '|' + (b.code || ''));
      }
    });
  }

  function blockDigest(b) {
  ...
  window.addEventListener('message', (e) => {
    ...
    else if (d.type === 'auto_send') { log('收到 auto_send 请求'); pasteToWebpageAI(d.text || ''); }
    else if (d.type === 'request_panel_side') {
      // iframe 就绪后主动询问当前挂靠侧，避免刷新后面板方向与记录不一致
      post({ type: 'panel_side', side: panelSide });
    }
    else if (d.type === 'set_panel_side') {
      panelSide = d.side === 'left' ? 'left' : 'right';
      applyPanelSide();
      try { chrome.storage.local.set({ aiMirrorPanelSide: panelSide }); } catch (e) {}
      log('set_panel_side: 挂靠侧 →', panelSide);
    }
    else if (d.type === 'close_panel') {
      setDialogVisible(false);
      log('close_panel: 已隐藏抽屉');
    }
  });
  ...
  getConfig((cfg) => {
    ...
    chrome.storage.local.get(['aiMirrorPanelSide'], (res) => {
      panelSide = (res && res.aiMirrorPanelSide === 'left') ? 'left' : 'right';
      dialogHidden = true;
      log('初始化：抽屉默认关闭，挂靠侧 =', panelSide);
      inject();
